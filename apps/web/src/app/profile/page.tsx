...
export default function ProfilePage() {
  ...
  const [ordersSubTab, setOrdersSubTab] = useState<'purchases' | 'sales'>('sales')
  ...
  useEffect(() => {
    if (activeTab === 'messages' && session && conversations.length === 0) {
      fetchConversations()
    }
  }, [activeTab, session])
  ...
  const handleCloseEditWine = () => {
    setEditingWine(null)
    setIsEditingWine(false)
  }

  const handleWineEditSuccess = () => {
    // Ricarica la lista dei vini dopo la modifica
    fetchUserWines()
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="border-b border-gray-200 mb-8">
          <nav className="-mb-px flex space-x-8">
            {[
              ...
              { id: 'sales', label: 'Ordini', icon: CurrencyEuroIcon },
              { id: 'messages', label: 'Messaggi', icon: ChatBubbleLeftRightIcon },
                    ...
                    <div key={order.id} className="border rounded-lg p-4 hover:bg-gray-50">
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                          {mainWine?.images && mainWine.images.length > 0 ? (
                            <Image
                              src={mainWine.images[0]}
                              alt={mainWine.title}
                              width={64}
                              height={64}
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-sm font-medium text-gray-900 truncate">
                            {mainWine?.title || 'Ordine'}
                          </h3>
                          <p className="text-sm text-gray-500">
                            Da: {order.seller.firstName} {order.seller.lastName}
                          </p>
                          <p className="text-sm text-gray-500">
                            Quantità: {totalQuantity} • {formatPrice(order.totalAmount)}
                          </p>
                        </div>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 text-sm text-gray-600">
                            ...
                            <span className="flex items-center">
                              ...
                              {order.orderItems?.reduce((sum, item) => sum + item.quantity, 0) || 0} articoli
                            </span>
            ...
            {ordersSubTab === 'purchases' && (
              <div className="space-y-8">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                  <div className="px-6 py-4 border-b border-gray-200">
                    <div className="flex items-center justify-between">
                      <h2 className="text-xl font-semibold text-gray-900">I Miei Acquisti</h2>
                      <Link
                        href="/orders"
                        className="text-wine-600 hover:text-wine-700 text-sm font-medium"
                      >
                  ...
                  <div className="divide-y divide-gray-200">
                    {orders.filter(order => order.seller && order.seller.id !== session?.user?.id).length === 0 ? (
                      <div className="text-center py-12">
                        <ShoppingBagIcon className="h-12 w-12 text-gray-300 mx-auto mb-4" />
                        <h3 className="text-lg font-medium text-gray-900 mb-2">
                      ...
                      orders.filter(order => order.seller && order.seller.id !== session?.user?.id).map((order) => (
                        <div key={order.id} className="p-6 hover:bg-gray-50 transition-colors">
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              <div className="flex items-center space-x-3 mb-2">
        ...
        {activeTab === 'messages' && (
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-gray-900">Le Mie Conversazioni</h2>
              <Link
                href="/messages"
                className="text-wine-600 hover:text-wine-700 text-sm font-medium"
              >
            ...
            {messagesLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">Non hai ancora messaggi</p>
                <Link
                  href="/browse"
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          {conv.unreadCount > 0 && (
                            <span className="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white bg-wine-600 rounded-full ml-2">
                              {conv.unreadCount}
                            </span>
                          )}
                        </div>
      ...
      {editingWine && (
        <EditWineModal
          wine={editingWine}
          isOpen={isEditingWine}
          onClose={handleCloseEditWine}
          onSuccess={handleWineEditSuccess}
        />
      )}
    </div>
  )
