...
export default function ManageOrderPage() {
  ...
  const fetchOrder = async () => {
    try {
      ...
      // Set initial newStatus to first possible status
      const possibleStatuses = getNextPossibleStatuses(data.status)
      if (possibleStatuses.length > 0) {
        setNewStatus(possibleStatuses[0])
      }
    } catch (error) {
  ...
  const updateOrderStatus = async () => {
    ...
    try {
      ...
      router.push('/orders?updated=true')
    } catch (error) {
  ...
  const getNextPossibleStatuses = (currentStatus: OrderStatus): OrderStatus[] => {
    switch (currentStatus) {
      ...
      case 'SHIPPED':
        // Seller cannot mark as DELIVERED - only buyer can
        return []
      default:
        return []
    }
