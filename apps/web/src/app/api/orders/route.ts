...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle status parameter - convert comma-separated string to multiple query params
    ...
    // If status is a comma-separated string, split it and add each as separate param
    if (status) {
      const statuses = status.split(',')
      statuses.forEach(s => otherParams.append('status', s.trim()))
    }
