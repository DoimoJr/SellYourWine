...
export default function WinePage() {
  ...
  const handleContactSeller = async () => {
    ...
    if (!wine?.seller?.id) {
      alert('Informazioni venditore non disponibili')
      return
    }

    console.log('🔍 Seller ID:', wine.seller.id)
    console.log('🔍 Wine object:', wine)

    try {
      // Crea conversazione con il venditore
      const payload = {
        recipientId: wine.seller.id,
        initialMessage: `Ciao, sono interessato al vino "${wine.title}". È ancora disponibile?`,
      }
      ...
      if (response.ok) {
        const data = await response.json()
        // Reindirizza alla pagina messaggi con la conversazione appena creata
        router.push(`/messages?conversation=${data.id}`)
      } else {
        const errorData = await response.json()
        console.error('Error response:', errorData)
        alert('Errore nella creazione della conversazione: ' + (errorData.message || 'Errore sconosciuto'))
      }
    } catch (error) {
