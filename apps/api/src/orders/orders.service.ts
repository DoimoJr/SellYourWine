...
import { calculateFees, calculateShipping } from '../config/fees.config';
...
export class OrdersService {
  ...
  async updateStatus(id: string, updateOrderStatusDto: UpdateOrderStatusDto, userId: string, userRole: string): Promise<any> {
    ...
    // Permission checks based on role and status transition
    const isAdmin = userRole === 'ADMIN' || userRole === 'SUPER_ADMIN';
    const isSeller = order.sellerId === userId;
    const isBuyer = order.buyerId === userId;

    if (!isAdmin) {
      // Check if user has any relationship to this order
      if (!isSeller && !isBuyer) {
        throw new ForbiddenException('You do not have permission to update this order');
      }

      // Buyer can only mark as DELIVERED when order is SHIPPED
      if (status === OrderStatus.DELIVERED) {
        if (!isBuyer) {
          throw new ForbiddenException('Only the buyer can mark order as delivered');
        }
        if (order.status !== OrderStatus.SHIPPED) {
          throw new BadRequestException('Order must be shipped before it can be marked as delivered');
        }
      }
      // Seller can update to SHIPPED or CANCELLED, but not DELIVERED
      else {
        if (!isSeller) {
          throw new ForbiddenException('Only the seller can update order to this status');
        }
      }
    }
    ...
    // Decrement wine quantity when seller puts order in PROCESSING
    if (status === OrderStatus.PROCESSING && order.status !== OrderStatus.PROCESSING) {
      await this.markWinesAsSold(updatedOrder.orderItems);
    }
  ...
  async processPayment(id: string, processPaymentDto: ProcessPaymentDto, userId: string): Promise<any> {
    ...
    // Wine quantity will be decremented when seller puts order in PROCESSING status
    // No need to decrement here on payment completion
  ...
  private async validateAndCalculateOrder(orderItems: { wineId: string; quantity: number }[]): Promise<any> {
    const wineItems = [];
  ...
  private calculateShippingForSeller(subtotal: number, itemCount: number): number {
    return calculateShipping(subtotal, itemCount);
  }
  ...
  async addToCart(userId: string, createOrderItemDto: { wineId: string; quantity: number }): Promise<any> {
    ...
    const existingItem = cart.orderItems.find(item => item.wineId === wineId);
    ...
    const cartTotal = updatedCart.orderItems.reduce((sum, item) => sum + (item.price.toNumber() * item.quantity), 0);
  ...
  async updateCartItem(userId: string, wineId: string, quantity: number): Promise<any> {
    ...
    const cartItem = cart.orderItems[0];
  ...
  async checkoutCart(userId: string, checkoutData: { shippingAddressId?: string; shippingAddress?: any; paymentProvider: string }): Promise<any> {
    ...
    if (carts.length === 0 || carts.every(cart => cart.orderItems.length === 0)) {
    ...
    for (const cart of carts) {
      if (cart.orderItems.length === 0) continue;

      // Calculate all fees for this seller's order
      const cartSubtotal = cart.orderItems.reduce((sum, item) => sum + (item.price.toNumber() * item.quantity), 0);
      const itemCount = cart.orderItems.reduce((sum, item) => sum + item.quantity, 0);

      const fees = calculateFees(cartSubtotal, itemCount);
